...
interface NewRefForm {
  shortRef: string;
  fullRef: string;
  url: string;
}

interface ModernReferencePickerProps {
  selectedReferences?: ModernReference[];
  onChange: (refs: ModernReference[]) => void;
}

const ModernReferencePicker = ({
  selectedReferences = [],
  onChange,
}: ModernReferencePickerProps) => {
  const [allRefs, setAllRefs] = useState<ModernReference[]>([]);
  const [newRef, setNewRef] = useState<NewRefForm>({ shortRef: '', fullRef: '', url: '' });

  useEffect(() => {
    ModernReferenceService.findAll()
      .then((res) => {
        if (Array.isArray(res.data)) {
          setAllRefs(res.data as ModernReference[]);
        } else {
  ...
  const handleAddExisting = (refId: string) => {
  ...
  const handleRemove = (refId: number) => {
  ...
  const handleCreateNew = async () => {
    try {
      ...
      const created = response.data as ModernReference;
      onChange([...selectedReferences, created]);
      setAllRefs([...allRefs, created]);
