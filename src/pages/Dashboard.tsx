...
const formatChartTooltipLabel = (entry: unknown): string =>
  formatTypeLabel(String((entry as { payload?: { name?: unknown } })?.payload?.name ?? ''));

/** Shared `pagebox`/`dashboard-container` wrapper for every Dashboard render branch. */
const DashboardPage = ({ busy, children }: { busy?: boolean; children: ReactNode }) => (
  <div className="pagebox dashboard-pagebox">
    <main className="dashboard-container" aria-busy={busy ? 'true' : undefined}>
      {children}
    </main>
  </div>
);
    ...
    return (
      <DashboardPage>
        ...
        <section className="dashboard-empty">
          ...
          <p>
            No data has been loaded into the database yet. Add sites and roads to see KPI totals and
            chart breakdowns.
          </p>
        </section>
                  ...
                  <PieChart>
                    ...
                    <Tooltip
                      formatter={(value, _name, entry) => {
                        const label = formatChartTooltipLabel(entry);
        ...
        >
          ...
          <div className="roads-explanation">
            ...
            <p>
              ...
              ancient routes. The underlying assumption is that the routes Roman roads used were not
              necessarily constructed &lsquo;ex novo&rsquo; but rather followed already existing
              prehistoric routes in the area. Confirmed stretches of road, for example in the
              Reichswald near Kleve, have been found near tumuli and even ancient field systems
              (&lsquo;Celtic Fields&rsquo;). It is therefore prudent to take these other temporal
              layers as well in better understanding the Roman infrastructural network.
            </p>
          </div>
        </section>
      </div>
